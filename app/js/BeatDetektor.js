...
BeatDetektor.prototype.process = function(timer_seconds, fft_data) {
	...
	if (this.ma_quality_total <= 0) {
	  this.ma_quality_total = 0.001;
  }
	if (this.ma_quality_avg <= 0) {
	  this.ma_quality_avg = 0.001;
  }
	...
	for (var contest_i in this.bpm_contest) {
		...
		if (this.bpm_contest[contest_i] > this.config.BD_FINISH_LINE/2.0) {
			...
			if (this.bpm_contest_lo[draft_int_lo] != this.bpm_contest_lo[draft_int_lo]) {
	...
	if (this.bpm_timer > this.winning_bpm/4.0 && this.current_bpm) {		
		...
		/*if ((this.beat_counter % 4) === 0) {
		  console.log("BeatDetektor("+this.BPM_MIN+","+this.BPM_MAX+"): [ Current Estimate: "+(winner/10)+" BPM ] [ Time: "+(parseInt(timer_seconds*1000.0)/1000.0)+"s, Quality: "+(parseInt(this.quality_total*1000.0)/1000.0)+", Rank: "+(parseInt(this.win_val*1000.0)/1000.0)+", Jitter: "+(parseInt(this.bpm_offset*1000000.0)/1000000.0)+" ]");
	  }*/
	}
